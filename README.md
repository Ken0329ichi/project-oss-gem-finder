...
[![Smithery](https://img.shields.io/badge/Smithery-10%20Tools%20Available-blueviolet?style=for-the-badge&logo=anthropic&logoColor=white)](https://smithery.ai/servers/ken0329ichi/oss-gems)
...
### 🛠️ One-Click MCP Installation (Claude Desktop, Cursor, Cline)

This service is published on **[Smithery.ai](https://smithery.ai/servers/ken0329ichi/oss-gems)** with 10 dynamic discovery tools.

#### Quick Install via Smithery CLI
```bash
# For Claude Desktop
npx -y @smithery/cli install @Ken0329ichi/oss-gems --client claude

# For Cursor / Cline
npx -y @smithery/cli install @Ken0329ichi/oss-gems --client cursor
```
...
```json
{
  "mcpServers": {
    "oss-gems": {
      "command": "npx",
      "args": ["-y", "@smithery/cli@latest", "run", "@Ken0329ichi/oss-gems", "--config", "{}"]
    }
  }
}
```
...
- **Pricing Tiers:**
  - `Tier 0 (Sample & MCP)`: Free (`GET /api/v1/gems/sample`, SSE/MCP tools)
...
### 🤖 Autonomous Agent Integration (Python / Web3 Example)

AI Agents can programmatically negotiate payments when encountering `402 Payment Required`:

```python
import requests
from web3 import Web3

BASE_GATEWAY = "https://kenichi.synology.me:8443/a2a/api/v1/gems"
USDC_BASE_ADDRESS = "0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913"

# 1. Request Data (Expect 402 Challenge)
resp = requests.get(BASE_GATEWAY)
if resp.status_code == 402:
    challenge = resp.json()
    pay_to = challenge["pay_to"]
    amount_usdc = challenge["price_usdc"]  # e.g., 0.005 USDC
    
    # 2. Autonomous Settlement on Base Network (ERC-20 transfer)
    # tx_hash = send_base_usdc(to=pay_to, amount=amount_usdc)
    # tx_hash = "0x..."
    
    # 3. Resubmit with Settlement Receipt
    headers = {"X-402-Tx-Hash": tx_hash}
    data_resp = requests.get(BASE_GATEWAY, headers=headers)
    print(data_resp.json())  # Data unlocked!
```
